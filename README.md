This is the code repository for "From Lockdown to Lexicon: How COVID-19 Changed Italy’s Health Parliamentary Discourse", a brief research project by Bernardoni Sara, Janssen Franz, Loreto Filippo, Monaco Gianmaria, Muriano Francesca for the course "20886 - Foundations of Social Sciences - Module I (Empirical Research Methods and Data Analysis)".

To replicate the results, please follow these steps:
1. Import the libraries from the ````prerequisites.py```` file. If they are not already installed on your machine, please run ````pip install```` followed by the name of the package in your terminal.
...
4.The file `utils.py` contains a helper function for saving plots used across multiple scripts in the `numeric analysis` folder. It is imported automatically; no separate execution is required.
